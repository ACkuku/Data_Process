...
from label2det import label2det
import cv2
"""
...
the iamges is a list, and each item is a dict which keys are file_name, height, width, id, depth;
...
def make_coco_annotations(src_json_file):
    ...
    for i in range(len(src_json_file)):
        ...
        eachAnnotationDict['segmentation'] = [[bbox[0], bbox[1], bbox[2], bbox[1], bbox[2], bbox[3], bbox[0], bbox[3]]]
        eachAnnotationDict['area'] = bbox_width * bbox_height
...
def convert_coco(args):
    src = json.load(open(args.src_path, 'r'))
    assert (type(src) == list), "unsupported type"
    cls_set = set()
    src = label2det(src, cls_set)
    ...
    json.dump(allInfo, open(save_path, 'w'), indent=4, separators=(',', ': '))
